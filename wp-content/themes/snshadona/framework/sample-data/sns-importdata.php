<?php
if ( ! function_exists( 'snshadona_importdata' ) ) {
	function snshadona_importdata(){
		$msg = '';
		...
		if( $datatype=='widget' ){
	        ...
	        ob_start();
			snshadona_import_widget($widget_data);
			ob_end_clean();
			update_option('snshadona_imported_widget', 1);
		}
		if( $datatype=='content' ){
			// Delete old menu and import new
			wp_delete_nav_menu('main-menu');
			wp_delete_nav_menu('vertical-menu');
			wp_delete_nav_menu('top-menu');
			wp_delete_nav_menu('about-us');
			wp_delete_nav_menu('company');
			wp_delete_nav_menu('help');
			wp_delete_nav_menu('more-info');
			// Import content
			$wp_import->import(get_template_directory() . '/framework/sample-data/data/all-content.xml');
			update_option('snshadona_imported_content',1);
		...
		if( $datatype=='theme' ){
			...
			snshadona_set_themeoptions($option_data);
		}
		ob_end_clean();
		if($datatype == 'widget'){
...
if ( ! function_exists( 'snshadona_set_themeoptions' ) ){
	function snshadona_set_themeoptions($option){
		$option = json_decode($option,true);
		update_option('snshadona_themeoptions',$option);
	}
}
if(!function_exists('snshadona_import_revslider')){
	function snshadona_import_revslider(){
...
if(!function_exists('snshadona_import_widget')){
	function snshadona_import_widget($import_array){
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
					...
					$new_widget_name 		= snshadona_widget_name( $title, $index );
...
if(!function_exists('snshadona_widget_name')){
	function snshadona_widget_name($widget_name, $widget_index){
